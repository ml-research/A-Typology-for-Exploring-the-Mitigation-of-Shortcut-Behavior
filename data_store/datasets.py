            ...
            if not fmnist:
                for i, e in enumerate(E):
                    ...
                    if det.item(0) in [x for x in range(0, 28)]:  # top row
                        ind = [738, 739, 740, 741, 742, 743, 744, 745,
                               766, 767, 768, 769, 770, 771, 772, 773]
                    else:  # bottom row
                        ind = [10, 11, 12, 13, 14, 15, 16, 17,
                ...
                for i, e in enumerate(E):
                    ...
                    if y[i] in [0, 1, 2, 3, 4, 6, 8]:  # rectangle left right
                        if det.item(0) in [0, 672]:  # left
                            ind = [306, 307, 334, 335, 362, 363, 390,
                                   391, 418, 419, 446, 447, 474, 475, 502, 503]
                        else:  # right
                            ind = [280, 281, 308, 309, 336, 337, 364,
                                   365, 392, 393, 420, 421, 448, 449, 476, 477]
                    else:  # rectangle bottom top row
                        if det.item(0) in [0, 24]:  # top
                            ind = [738, 739, 740, 741, 742, 743, 744,
                                   745, 766, 767, 768, 769, 770, 771, 772, 773]
                        else:  # bottom
                            ind = [10, 11, 12, 13, 14, 15, 16, 17,
                                   38, 39, 40, 41, 42, 43, 44, 45]
                    cur = np.zeros(d, dtype=np.int64)
    ...
    if hint_expl:
        if n_expl is not None:
            train, test = TensorDataset(
                X, y, E_hint, flags), TensorDataset(Xt, yt, Et)
        else:
            train, test = TensorDataset(
                X, y, E_hint), TensorDataset(Xt, yt, Et)
        return DataLoader(train, batch_size, train_shuffle), DataLoader(test, batch_size, test_shuffle)
...
def decoy_mnist_retrain(elem_num, fmnist=False, batch_size=256, device='cuda',
    ...
    if n_expl is not None:
        not_used_flags = np.zeros(
            (E_retrain.shape[0] - n_expl), dtype=np.int64)
        used_flags = np.ones(n_expl, dtype=np.int64)
        flags = np.concatenate((used_flags, not_used_flags), axis=0)
    ...
    if hint_expl:
        if n_expl is not None:
            train, test = TensorDataset(
                X_retrain, y_retrain, E_hint_retrain, flags), TensorDataset(Xt, yt, Et)
        else:
            train, test = TensorDataset(
                X_retrain, y_retrain, E_hint_retrain), TensorDataset(Xt, yt, Et)
        return DataLoader(train, batch_size, train_shuffle), DataLoader(test, batch_size, test_shuffle)

    if n_expl is not None:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain, flags), TensorDataset(Xt, yt, Et)
    else:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain), TensorDataset(Xt, yt, Et)
...
def decoy_mnist_both(no_decoy=False, fmnist=False, batch_size=256, device='cuda',
    ...
    if n_expl is not None:
        train, test = TensorDataset(
            X, y, E, E_hint, flags), TensorDataset(Xt, yt, Et)
    else:
...
def decoy_mnist_both_retrain(elem_num, fmnist=False, batch_size=256, device='cuda',
    ...
    if n_expl is not None:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain, E_hint_retrain, flags), TensorDataset(Xt, yt, Et)
    else:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain, E_hint_retrain), TensorDataset(Xt, yt, Et)
...
def decoy_mnist_CE_combined(fmnist=False, batch_size=256, device='cuda',
    ...
    X_corrections, y_corrections = ce.get_corrections(
        X, E_ce, y, n_instances, n_counterexamples_per_instance, ce_strategy)
    # add to train set
    X = np.vstack([X, X_corrections])
    y = np.hstack([y, y_corrections])
    print(f"Train set was augmented: X.size= {len(X)}, y.size= {len(y)}")
    ...
    if hint_expl:
        if n_expl is not None:
            train, test = TensorDataset(
                X, y, E_hint, mask, flags), TensorDataset(Xt, yt, Et)
        else:
            train, test = TensorDataset(
                X, y, E_hint, mask), TensorDataset(Xt, yt, Et)
        return DataLoader(train, batch_size, train_shuffle), DataLoader(test, batch_size, test_shuffle)

    if n_expl is not None:
        train, test = TensorDataset(
            X, y, E, mask, flags), TensorDataset(Xt, yt, Et)
    else:
...
def decoy_mnist_CE_combined_retrain(elem_num, fmnist=False, batch_size=256, device='cuda',
    ...
    if device == 'cpu':
        ...
        mask_retrain = torch.from_numpy(
            mask_retrain).type(torch.cuda.LongTensor)
    ...
    if hint_expl:
        if n_expl is not None:
            train, test = TensorDataset(
                X_retrain, y_retrain, E_hint_retrain, mask_retrain, flags), TensorDataset(Xt, yt, Et)
        else:
            train, test = TensorDataset(
                X_retrain, y_retrain, E_hint_retrain, mask_retrain), TensorDataset(Xt, yt, Et)
        return DataLoader(train, batch_size, train_shuffle), DataLoader(test, batch_size, test_shuffle)

    if n_expl is not None:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain, mask_retrain, flags), TensorDataset(Xt, yt, Et)
    else:
        train, test = TensorDataset(
            X_retrain, y_retrain, E_retrain, mask_retrain), TensorDataset(Xt, yt, Et)
...
def decoy_mnist_all(fmnist=False, batch_size=256, device='cuda', train_shuffle=False,
    ...
    if n_expl is not None:
        train, test = TensorDataset(
            X, y, E, E_hint, mask, flags), TensorDataset(Xt, yt, Et)
    else:
        train, test = TensorDataset(
            X, y, E, E_hint, mask), TensorDataset(Xt, yt, Et)
...
def isic_2019(batch_size=16, train_shuffle=True, number_nc=None, number_c=None,
    ...
    if ce_augment:
        ...
        for idx in train_dataset.indices:
            if complete_dataset[idx][3] == 1:
                ...
                number += 1
        ce_dataset = TensorDataset(torch.cat(img_ce, 0), torch.Tensor(labels_ce).type(
            torch.LongTensor), torch.cat(masks_ce, 0), torch.cat(flags_ce))
        ...
        if len(ce_dataset) + length_before_ce_augmentation != len(train_dataset):
            RuntimeWarning(
                f"Length of trainset before and after ce augmentation incorrect!")
    ...
    logging.info(
        f"  TEST_NO_PATCHES class dist: {Counter(test_classes_no_patches)}")
...
def isic_2019_hint(batch_size=16, train_shuffle=True, number_nc=None, number_c=None,
    ...
    logging.info(
        f"  TEST_NO_PATCHES class dist: {Counter(test_classes_no_patches)}")
    logging.info(
        f"  TEST NO PATCHES masks dist: {Counter(test_masks_no_patches)}")
    logging.info(f"  Loss weights: {str(weights)}")
...
def decoy_mnist_all_revised(fmnist=False, batch_size=256, device='cuda', train_shuffle=False,
    ...
    if generate_counterexamples:
        X_corrections, y_corrections = ce.get_corrections(
            X, E_ce, y, n_instances, n_counterexamples_per_instance, counterexample_strategy)
        ...
        logging.info(
            f"Train set was augmented with counterexamples: X.size= {len(X)}, y.size= {len(y)}")

        # first 60000 are non CE, next 60000 are CE
        counterexample_mask = np.append(np.zeros(60_000, dtype=int), np.ones(
            60_000, dtype=int))  # 0 = non CE, 1 = CE
    ...
    counterexample_mask = torch.from_numpy(
        counterexample_mask).bool().to(device)

    if n_expl:
        flags = torch.from_numpy(flags).to(device)

    logging.debug(
        f"X.shape={X.shape}, Xt.shape={Xt.shape}, y.shape={y.shape}, yt.shape={yt.shape}")

    if n_expl:
        train, test = TensorDataset(
            X, y, E_pnlt, E_rwrd, counterexample_mask, flags), TensorDataset(Xt, yt, Et)
    else:
        train, test = TensorDataset(
